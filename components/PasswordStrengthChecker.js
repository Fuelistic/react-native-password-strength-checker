...
export default class PasswordStrengthChecker extends Component {
	...
	isMatchingRules(password) {
		...
		const rules = _.chain(ruleNames).split('|').filter((rule) => !!rule).map((rule) => rule.trim()).value();
	...
	renderPasswordStrength() {
		...
		return (
			<View style={[ styles.passwordStrengthWrapper, strengthWrapperStyle ]}>
				...
				<Text style={[ styles.strengthDescription, strengthDescriptionStyle, { ...strengthLevelLabelStyle } ]}>
					{strengthLevelLabel}
				</Text>
			</View>
		);
